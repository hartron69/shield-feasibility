...
export default function OperationalInputsPanel() {
  const [activeSite, setActiveSite] = useState(OPERATIONAL_SITE_IDS[0])
  const site = MOCK_OPERATIONAL_INPUTS[activeSite]

  return (
    <div className="bio-panel">
      <div className="bio-site-row">
        {OPERATIONAL_SITE_IDS.map(sid => (
          <button
            key={sid}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <span style={{ fontWeight: 600, fontSize: 13 }}>
          {site.site_name} — Operational Indicators
        </span>
        ...
        <tbody>
          {site.readings.map((row, i) => (
            <tr key={i} className={row.adverse ? 'bio-row-adverse' : ''}>
              ...
              <td className={row.adverse ? 'delta-bad' : 'delta-good'}>
